...
if __name__ == '__main__':
    ...
    model = YOLO(r"D:\autumn\Documents\GitHub\ultralytics\ultralytics\yolo_workspace\train_output\bird\train\train3\weights\best.pt")
    # Run inference on 'bus.jpg' with arguments
    results = model.predict(
        # source="/media/autumn/新加卷/现场识别错误数据_1",  # 指定推理的数据源。可以是图像路径、视频文件、目录、URL 或用于实时馈送的设备 ID。
        source=r"D:\autumn\Downloads\birds.mp4",  # 指定推理的数据源。可以是图像路径、视频文件、目录、URL 或用于实时馈送的设备 ID。
        ...
        project="bird/test",  # 保存预测结果的项目目录名称，如果 `save` 已启用。
